    ...
    <div className="p-6 max-w-3xl">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Custom domain cho tracking links của publisher — tách biệt khỏi web app</p>
          </div>
          <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${trackingDomain ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
            {trackingDomain ? 'Đã cấu hình' : 'Chưa cấu hình'}
          </span>
        </div>
        ...
        <div className="px-6 py-5">
          {trackingDomain ? (
            <div className="mb-5">
              <p className="text-xs text-gray-500 mb-1 font-medium">Tracking domain hiện tại:</p>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-3 mb-5 text-sm text-yellow-800">
              <p className="font-medium mb-1">Chưa có tracking domain riêng</p>
              <p className="text-xs text-yellow-700">Tracking links đang dùng chung domain web app. Làm theo hướng dẫn bên dưới để cấu hình domain riêng.</p>
            </div>
          )}
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
              <h3 className="text-sm font-semibold text-gray-800">Hướng dẫn cấu hình Custom Tracking Domain</h3>
              <p className="text-xs text-gray-500 mt-0.5">Thiết lập domain riêng trỏ thẳng vào API server, không qua web app</p>
            </div>
            <div className="px-4 py-5 space-y-5">
              <Step n={1}>
                <strong>Mua/chọn domain tracking</strong> — ví dụ: <code className="font-mono text-xs bg-gray-100 px-1 rounded">trk.yourdomain.com</code>. Nên dùng subdomain của domain chính hoặc một domain hoàn toàn mới. Tránh dùng tên liên quan đến "affiliate", "track", "click" quá rõ ràng.
              </Step>

              <Step n={2}>
                <div>
                  <strong>Thêm Custom Domain vào Railway API service</strong> (không phải web service):
              ...
              <Step n={3}>
                <div>
                  <strong>Trỏ DNS về Railway API</strong>:
                  ...
                  <p className="text-xs text-gray-500 mt-2">CNAME target lấy từ bước 2 ở màn hình Railway. Chờ DNS propagate 5–30 phút.</p>
                </div>
              </Step>

              <Step n={4}>
                <div>
                  <strong>Set env var <code className="font-mono text-xs">TRACKING_DOMAIN</code> trên Railway Web service</strong>:
                  <p className="text-xs text-gray-500 mt-1 mb-1">Vào Railway → Web service → Variables → thêm:</p>
                  <CodeLine>TRACKING_DOMAIN=https://trk.yourdomain.com</CodeLine>
                  <p className="text-xs text-gray-500 mt-2">Railway sẽ tự redeploy web app. Sau khi deploy xong, tracking links của publisher sẽ dùng domain mới.</p>
                </div>
              </Step>

              <Step n={5}>
                <div>
                  <strong>Cập nhật CORS trên Railway API service</strong> (nếu cần):
                  <p className="text-xs text-gray-500 mt-1 mb-1">Nếu <code className="font-mono text-xs">CORS_ORIGIN</code> đang set giá trị cụ thể, thêm web domain vào:</p>
                  <CodeLine>CORS_ORIGIN=https://web.railway.app</CodeLine>
                  <p className="text-xs text-gray-400 mt-2">Tracking domain không cần CORS — click redirect là browser request, không phải AJAX.</p>
                </div>
              </Step>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-0.5">URL đăng ký với MMP — dùng API domain, không dùng tracking domain</p>
        </div>
        <div className="px-6 py-5 space-y-3 text-sm">
          ...
          <p className="text-xs text-gray-400 pt-1">Tham khảo file <code className="font-mono">docs/huong-dan-tich-hop-mmp.md</code> để biết thêm về các tham số postback.</p>
        </div>
      </div>
    </div>
  )
